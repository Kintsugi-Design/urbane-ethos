...
- `chatbot.js` — scripted decision tree (no LLM), bilingual, lazy-builds the panel on launcher click. **No voice.** The Web Speech API mic and the SpeechSynthesis read-aloud toggle were removed on 2026-08-14 (client decision): the mic had no `onerror`, no listening state and never submitted its transcript, so a denied permission failed silently; the TTS toggle did work but never changed its icon, so it read as dead. Don't reinstate either without also building the missing feedback states.
...
- `icons.js` — inline SVG sprite map (`data-icon="name"` → Heroicons outline, plus a filled `whatsapp` brand mark). All entries carry `aria-hidden="true"`. **Eight glyphs**, every one in use: `chat`, `x-mark`, `chevron-up-down`, `phone`, `envelope`, `map-pin`, `send`, `whatsapp`. (`speaker`, `speaker-muted` and `microphone` were deleted with the chatbot voice controls on 2026-08-14 — `speaker-muted` had never been referenced at all.) Keep it that way: a glyph with no consumer is dead weight.
...
3. `assets/img/staff-pdf/` — 5 **low-res interim headshots** extracted from the company-profile PDF, flagged `"photoInterim": true`.
...
- **Fetching `content/` from a JS module: resolve against `import.meta.url`, not the document.** A bare `fetch("content/x.json")` is *document*-relative, so it only works for pages sitting at the deploy root. Every production page does, which is why this hid for months — but it silently 404s from any subdirectory, and that is precisely what broke `test/smoke/i18n.html` and `test/smoke/chatbot.html` into rendering **zero assertions** (the loader rejected before the first assertion ran, so the page reported neither PASS nor FAIL — it looked fine). Use `new URL("../../content/…", import.meta.url)` as `i18n.js`, `chatbot.js` and `map-embed.js` all now do: the module's position relative to `content/` is fixed, so it is correct at the repo root, at a custom-domain root, and at a repo subpath, and it introduces no root-absolute literal. **No document-relative content fetch remains in `assets/js/` — keep it that way.** Check with `grep -rn 'fetch(' assets/js/ | grep -v import.meta.url`, which should return only `i18n.js`'s fetch of an already-resolved `CONTENT_BASE` URL.
- **Service copy lives in four places — change all of them together.** `content/{en,ms}/services.json` `items[].whatItIs` is the source of truth, but the same copy is mirrored into (1) the `services.html` SEO static fallback, (2) the chatbot's `flow["service.<key>"].say` in `content/{en,ms}/chatbot.json`, and — if the service also appears on the home grid — (3) `content/{en,ms}/home.json` `services.items[].blurb` plus (4) its `index.html` mirror. **No automated gate catches this drift.** The two HTML fallbacks are at least mechanically checkable against their JSON, but the chatbot and home strings are hand-written compressed paraphrases, so nothing ties them back to the source of truth — the 2026-08-21 copy refresh corrected `services.json` and left all three of the others stale until a code review caught it.
...
- **GitHub Pages** (live): `.github/workflows/pages.yml` deploys on push to `main`. Target remote `git@github.com:Kintsugi-Design/urbane-ethos.git`. **Public URL is `https://kintsugi.my/urbane-ethos/`** — a custom domain. `https://kintsugi-design.github.io/urbane-ethos/` still resolves but **301-redirects** there, so any script or check that hits the `github.io` host must follow redirects (`curl -L`) or it reads the redirect stub and sees an empty page rather than the site. The domain is configured in the repo's Pages settings, not by a `CNAME` file in the tree — don't add one, and don't "fix" its absence. Confirm with `gh api repos/Kintsugi-Design/urbane-ethos/pages`. First-run Pages enablement is automated via `actions/configure-pages@v5` with `enablement: true`.
