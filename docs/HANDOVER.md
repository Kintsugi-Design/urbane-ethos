...
- ~~`assets/js/map-embed.js:31` still fetches `./content/…` document-relative~~ — **fixed**, so all three modules now use the same idiom and no document-relative content fetch remains anywhere in `assets/js/`. A/B verified rather than assumed: a probe page under `test/smoke/` loads the map with the fix and silently fails without it (`mapState` resets to `""`, no iframe, no console error — the module's own retry path masks it). Root and repo-subpath deploys both still load the map on index and contact.
