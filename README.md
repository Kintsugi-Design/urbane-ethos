...
assets/
  ...
  js/           i18n consent chatbot personalization nav a11y sage-stamp
                ...
                map-embed storage enquiry                      (16 modules)
...
bin/
  ...
  check-i18n-parity.rb         EN/MS parity check (stdlib only)          — CI gate 1
  check-contact-channels.rb    mailto:/tel:/wa.me drift check (stdlib)   — CI gate 2
test/
...
**Mocked (interactive but not wired to a backend):** chatbot replies (scripted decision tree, no LLM), personalization rules (hard-coded concern→service map in `personalization.js`), staff intro videos (placeholder `<dialog>` / `data-yt-id` slots), analytics data (seeded fake data in `analytics-demo-data.js`), and **contact form submission — still no backend.**

**How the contact form actually delivers (updated 2026-08-09).** Submitting no longer navigates straight to a `mailto:`. It validates, then swaps the form for a success panel offering **three** routes, all carrying the identical composed message:

- **Continue on WhatsApp** — a real `wa.me` deep link (the number is derived from `content/en/contact.json`, never hard-coded).
- **Send by email instead** — a `mailto:` with the subject and body pre-filled. *This* is the only path that opens a mail app.
- **Copy message** — clipboard fallback for devices with neither handler registered.

Nothing navigates until the visitor picks one, so no popup blocker can intercept it. There is still **no server, no database and no delivery guarantee** — the visitor's own mail or WhatsApp client does the sending. A real backend remains out of scope.

The form also pre-fills from signals the site already holds (`?service=` from a services CTA, chatbot context, personalization survey) and shows a visible "We've filled some of this in… · Clear" line when it does. The concern textarea is **never** pre-filled — it holds sensitive child-health free text.

**Clear my data.** Every page footer (and the consent modal) carries a "Clear my data" control that opens a native `<dialog>`, enumerates the five things it wipes, and on confirm removes every `urbane-ethos:*` key from both `localStorage` and `sessionStorage` plus the legacy unprefixed `session-chat`, then reloads. It is labelled "data", not "cookies", because the site sets **zero** cookies — all state is web storage.
...
Two Ruby stdlib checks, run **in this order** by both `.github/workflows/pages.yml` (job `ci`) and `.gitlab-ci.yml` (job `content-gates`). Keep the two pipelines identical.
...
**1. i18n parity.** Exit 0 = all keys mirrored between `content/en/` and `content/ms/` (9 namespaces). The `_placeholder` maps **are** walked and must mirror key-for-key; only `_meta` / `_draft` / `_correction` are skipped. `blog.json` and `careers.json` are root-level and intentionally EN-only (parity-exempt).

**2. Contact-channel drift.** Exit 0 = every literal `mailto:` / `tel:` / `wa.me` in the shipped surfaces agrees with the content files. The allowlist is built **from content, never hard-coded**, and the scan covers every root `*.html` (including the 38 generated `post-*.html`), `content/blog/_post.html.erb`, and `content/blog/posts/*.md`. Template expressions (`mailto:${…}`, `<%= … %>`) are skipped, not failed.

This second gate exists because the first kind of check cannot catch the second kind of bug: a `content/`-only grep once "verified" that a placeholder address was gone while `info@urbaneethos.center` sat hard-coded in a `contact.html` inline script, silently receiving every enquiry. **Change a number or address in `content/en/contact.json` and this gate tells you every page that disagrees.**
